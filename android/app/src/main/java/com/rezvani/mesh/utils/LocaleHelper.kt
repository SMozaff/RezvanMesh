...
object LocaleHelper {
    ...
    /**
     * Inspects the device's PRIMARY configured locale (the phone's actual
     * display-language setting) and returns "fa" only if that primary locale
     * is Farsi, otherwise "en".
     *
     * Bug fix: this previously scanned the ENTIRE configured locale list
     * (Configuration.locales) and returned "fa" if Farsi appeared ANYWHERE
     * in it, even as a secondary/keyboard-input locale. Many phones list
     * more than one locale (e.g. a secondary keyboard language) without
     * that being the user's actual display language -- so a phone whose
     * display language was English, but which had Farsi present anywhere
     * in its locale list, would incorrectly launch in Farsi. Only the
     * primary locale (index 0) reflects what the user actually chose as
     * their device's display language, so only that one is checked here.
     */
    private fun detectSystemLanguage(context: Context): String {
        val configLocales = context.resources.configuration.locales
        if (configLocales.isEmpty) return "en"
        return if (configLocales[0].language == "fa") "fa" else "en"
    }
